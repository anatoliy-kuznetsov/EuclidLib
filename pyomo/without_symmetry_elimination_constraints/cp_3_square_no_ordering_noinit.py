...
m.e3 = Constraint(expr= (m.x2 - m.x3)**2 + (m.x5 - m.x6)**2 - (2 * m.x7)**2
    >= 0)
m.e4 = Constraint(expr= m.x1 + m.x7 <= 0.5)
m.e5 = Constraint(expr= m.x2 + m.x7 <= 0.5)
m.e6 = Constraint(expr= m.x3 + m.x7 <= 0.5)
m.e7 = Constraint(expr= m.x1 - m.x7 >= -0.5)
m.e8 = Constraint(expr= m.x2 - m.x7 >= -0.5)
m.e9 = Constraint(expr= m.x3 - m.x7 >= -0.5)
m.e10 = Constraint(expr= m.x4 + m.x7 <= 0.5)
m.e11 = Constraint(expr= m.x5 + m.x7 <= 0.5)
m.e12 = Constraint(expr= m.x6 + m.x7 <= 0.5)
m.e13 = Constraint(expr= m.x4 - m.x7 >= -0.5)
m.e14 = Constraint(expr= m.x5 - m.x7 >= -0.5)
m.e15 = Constraint(expr= m.x6 - m.x7 >= -0.5)
